...
function Columns() {
  ...
  const dragend = (results) => {
    ...
    if (results.source.droppableId === results.destination.droppableId) {
      let targetColumn = [...defaultboard.columns].find(
        (column) => column.id == results.source.droppableId
      );
      let mytask = targetColumn.tasks.find(
        (task) => task.id == results.draggableId
      );
      if (results.source.index !== results.destination.index) {
        let Newtasks = targetColumn.tasks.filter(
          (task) => task.id !== results.draggableId
        );
        ...
        let newcolumns = [...defaultboard.columns].map((column) =>
          column.id === results.source.droppableId ? targetColumn : column
        );
        setDefaultboard({ ...defaultboard, columns: newcolumns });
      }
    } else {
      let mytask = [...defaultboard.columns]
        .find((column) => column.id == results.source.droppableId)
        .tasks.find((task) => task.id == results.draggableId);
      ...
      let addtasktoColums = removetaskfromColumns.map((column) => {
        if (column.id == results.destination.droppableId) {
          ...
          Newtasks.splice(results.destination.index, 0, {
            ...mytask,
            status: column.name,
            statusId: column.id,
          });
              ...
              {(provided, snapshot) => {
                return (
                  <div
                    ...
                    className="column"
                    style={{
                      border: `3px dashed ${colors[index]}`,
                      borderBottom: `1px solid transparent`,
                    }}
                  >
                    <div
                      className="column-circle"
                      style={{ borderBottom: `3px dashed ${colors[index]}` }}
                    >
                      {" "}
                      <div
                        className="circle"
                      ...
                      ></div>{" "}
                      <p key={column.id} className="column-title">
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  minWidth: snapshot.isDragging ? "270px" : "",
                                  maxWidth: snapshot.isDragging ? "270px" : "",
