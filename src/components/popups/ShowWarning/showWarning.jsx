  ...
  const deletejob = () => {
    if (showwarning.task) {
      ...
      let NewColumns = defaultboard.columns.map((column) => {
        if (
          column.name == showwarning.task.status ||
          column.id == showwarning.task.statusId
        ) {
          return {
            ...column,
            tasks: column.tasks.filter(
              (Task) => Task.id != showwarning.task.id
            ),
          };
        }
        return column;
    ...
    >
      <div className="warning">
        ...
        <p>
          ...
          <span className="Board-title">{showwarning.title}</span>' board? This
